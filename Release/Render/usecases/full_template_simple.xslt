...
-->
  ...
  <table class="cols">
	...
	<xsl:template match="operation">
		...
		<td>
		RQ:<xsl:apply-templates select="./request" />
		
		</td>
		<td>
		RP:<xsl:apply-templates select="./response" />
		</td>
	</xsl:template>
...
<xsl:if test="expression">
		...
		<xsl:variable name="thisResourceId" select="./resourceId/@value" />
		<xsl:variable name="versions" select="count(./version)" />
...
(<xsl:value-of select="$versions"/>)
			...
			<td>
				<b><xsl:value-of select="description/@value" /></b>
			</td>
			<td  colspan="3">
	...
	<xsl:template match="version">
		...
		<tr>
			<td>
		<xsl:value-of select="versionId/@value" />
			</td>
			<td>
		<xsl:value-of select="description/@value" />
			</td>
